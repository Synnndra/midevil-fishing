...
async function redisSet(key, value) {
    const response = await fetch(`${UPSTASH_URL}/set/${key}/${value}`, {
        headers: { Authorization: `Bearer ${UPSTASH_TOKEN}` }
    });
...
async function redisSetEx(key, seconds, value) {
    const response = await fetch(`${UPSTASH_URL}/setex/${key}/${seconds}/${encodeURIComponent(value)}`, {
        headers: { Authorization: `Bearer ${UPSTASH_TOKEN}` }
    });
    return response.json();
}
...
async function redisDecr(key) {
    const response = await fetch(`${UPSTASH_URL}/decr/${key}`, {
        headers: { Authorization: `Bearer ${UPSTASH_TOKEN}` }
    });
    const data = await response.json();
    return data.result;
...
async function redisExpire(key, seconds) {
    const response = await fetch(`${UPSTASH_URL}/expire/${key}/${seconds}`, {
        headers: { Authorization: `Bearer ${UPSTASH_TOKEN}` }
    });
    return response.json();
}
...
async function redisLpush(key, value) {
    const response = await fetch(`${UPSTASH_URL}/lpush/${key}/${encodeURIComponent(value)}`, {
        headers: { Authorization: `Bearer ${UPSTASH_TOKEN}` }
    });
    return response.json();
}
